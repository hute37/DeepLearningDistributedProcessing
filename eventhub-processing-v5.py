...
if __name__ == "__main__":
    ...
    def process(time, rdd):
        ...
        if rdd.isEmpty() != True: 
            # rdd.map(lambda dictobj: dictobj.keys()).collect()   
            # print (rdd.map(lambda dictobj: dictobj.keys()).collect())         
            # print (rdd.map(lambda dictobj: dictobj.get("body_").decode("utf-8")).collect())
            print (rdd.map(lambda dictobj: str(dictobj.get("body_"))).collect())         
